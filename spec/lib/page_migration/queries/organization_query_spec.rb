...
RSpec.describe PageMigration::Queries::OrganizationQuery do
  let(:org_ref) { "Pg4eV6k" }
  let(:query) { described_class.new(org_ref) }

  describe "#call" do
    let(:mock_conn) { double("connection") }
    let(:result_data) { {"data" => '{"organizations": []}'} }
    ...
    context "when organization exists" do
      ...
      it "executes the SQL query with org_ref" do
        expect(mock_conn).to receive(:exec_params)
          .with(PageMigration::Queries::OrganizationSql::SQL, [org_ref])
        query.call(mock_conn)
      end

      it "returns the data from the result" do
        expect(query.call(mock_conn)).to eq('{"organizations": []}')
      end
    end
