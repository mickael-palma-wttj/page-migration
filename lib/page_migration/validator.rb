  ...
  module Validator
    ...
    class << self
      ...
      def validate_file_exists!(path)
        raise Errors::FileNotFoundError.new("File not found: #{path}", file_path: path) unless File.exist?(path)

        path
      end

      def validate_directory_exists!(path)
        raise Errors::FileNotFoundError.new("Directory not found: #{path}", file_path: path) unless Dir.exist?(path)
