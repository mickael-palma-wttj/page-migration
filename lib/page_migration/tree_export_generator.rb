...
module PageMigration
  ...
  class TreeExportGenerator
    ...
    def generate
      ...
      # Start from the real root pages of the organization to preserve hierarchy
    ...
    def export_node(tree_page, current_path)
      ...
      # For the absolute root "/", we stay at the current path
      # For others, we create a subfolder based on the slug
    ...
    def render_page_content(tree_page, page_data)
      # Reusing logic from FullExportGenerator but simplified for a single file
      buffer = []
